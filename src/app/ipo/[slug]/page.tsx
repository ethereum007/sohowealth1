...
export default async function IpoAnalysisPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-white pb-20">
    ...
    <section className="border-y border-slate-200 bg-slate-50 px-6 py-10"><div className="container mx-auto max-w-5xl"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Price band",item.issue.priceBand],["Issue size",money(item.issue.totalCr)],["Fresh issue",money(item.issue.freshCr)],["Offer for sale",money(item.issue.ofsCr)],["Open",item.issue.open],["Close",item.issue.close],["Listing",item.issue.listing],["Minimum lot",item.issue.lotSize ? `${item.issue.lotSize} shares` : "Not yet announced"]].map(([label,value]) => <div key={label} className="rounded-xl bg-white p-5"><p className="font-body text-xs font-bold uppercase tracking-[0.1em] text-slate-500">{label}</p><p className="mt-2 font-body text-base font-bold text-slate-900">{value}</p></div>)}</div>{item.issueNote && <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 font-body text-xs leading-relaxed text-amber-900">Offer-term note: {item.issueNote}</p>}</div></section>
    <article className="container mx-auto max-w-5xl space-y-16 px-6 py-16">
      ...
      <section><h2 className="font-display text-3xl font-semibold" style={{ color: "#0B1F3A" }}>{item.company} IPO FAQs</h2><div className="mt-6 divide-y divide-slate-200 rounded-xl border border-slate-200">{faqs.map(faq => <div key={faq.question} className="p-6"><h3 className="font-body text-base font-bold text-slate-900">{faq.question}</h3><p className="mt-2 font-body text-sm leading-relaxed text-slate-600">{faq.answer}</p></div>)}</div></section>
      ...
      <nav aria-label="Related IPO research" className="rounded-xl bg-slate-50 p-6"><h2 className="font-display text-2xl font-semibold text-slate-900">Continue your IPO research</h2><div className="mt-4 flex flex-wrap gap-3 font-body text-sm font-semibold"><Link href="/ipo" className="rounded-full bg-white px-4 py-2 text-emerald-800">Upcoming IPO calendar</Link><Link href="/ipo/methodology" className="rounded-full bg-white px-4 py-2 text-emerald-800">IPO analysis methodology</Link><Link href="/ipo/learn" className="rounded-full bg-white px-4 py-2 text-emerald-800">IPO learning centre</Link><Link href="/ipo/tools/subscription-explainer" className="rounded-full bg-white px-4 py-2 text-emerald-800">Subscription explained</Link></div></nav>
