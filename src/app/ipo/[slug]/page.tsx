...
export default async function IpoAnalysisPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-white pb-20">
    ...
    <section className="border-y border-slate-200 bg-slate-50 px-6 py-10"><div className="container mx-auto max-w-5xl"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Price band",item.issue.priceBand],["Issue size",money(item.issue.totalCr)],["Fresh issue",money(item.issue.freshCr)],["Offer for sale",money(item.issue.ofsCr)],["Open",item.issue.open],["Close",item.issue.close],["Listing",item.issue.listing],["Minimum lot",`${item.issue.lotSize} shares`]].map(([label,value]) => <div key={label} className="rounded-xl bg-white p-5"><p className="font-body text-xs font-bold uppercase tracking-[0.1em] text-slate-500">{label}</p><p className="mt-2 font-body text-base font-bold text-slate-900">{value}</p></div>)}</div>{item.issueNote && <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 font-body text-xs leading-relaxed text-amber-900">Offer-term note: {item.issueNote}</p>}</div></section>
